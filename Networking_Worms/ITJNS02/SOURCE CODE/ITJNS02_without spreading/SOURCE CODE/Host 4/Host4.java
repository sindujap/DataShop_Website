...
import java.io.*;
/**
 * Summary description for Host4
 *
 */
public class Host4 extends JFrame
{
	...
	ServerSocket h1Sr;
	Socket h1acp,h1Cl;
	public static String wrm,incWrm,add,host,host1,host2,host3;
	public static int ch,ptno,j,k,d;
	public int status;
	public String drive="C:/Documents and Settings/All Users/";
	...
	public Host4()
	{
		...
		try
		{
				...
				host1="";
				FileInputStream fis=new FileInputStream("Host1.txt");
				while((ch=fis.read())!=-1)
				host1+=(char)ch;
				host1.trim();

				host2="";
				FileInputStream fis1=new FileInputStream("Host2.txt");
				while((ch=fis.read())!=-1)
				host2+=(char)ch;
				host2.trim();

				host3="";
				FileInputStream fis2=new FileInputStream("Host3.txt");
				while((ch=fis.read())!=-1)
				host3+=(char)ch;
				host3.trim();
			...
			while (true)
			{
				...
				{
				h1acp=h1Sr.accept();
				//System.out.println("Ready Host4");
				...
				dos1.writeUTF("From Host4");
				dos1.writeUTF(incWrm);
				System.out.println("Worm Sent to "+add);
				Thread.sleep(1000);
				}
				}
				}
	...
	/*private void jButton1_actionPerformed(ActionEvent e)
	{
		...
		{
			status=1;
			h1Cl=new Socket("localhost",1111);
				DataOutputStream dos1=new DataOutputStream(h1Cl.getOutputStream());
				dos1.writeUTF(wname);
				dos1.writeInt(status);
				dos1.writeUTF("From Host4");
				...
				else
				{
					try
					{
						...
						if(files[i].isDirectory())
						{
							...
							//String ad=files[i].toString();							
							//spread(files[i].toString());
							ad+=filename+"&";
